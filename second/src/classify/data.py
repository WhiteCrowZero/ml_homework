...
class ImageClassificationDataset:
    ...
    def __init__(
        self, root_folder: str, num_classes: int = 20, dataset_name="tinyimagenet"
    ) -> None:
        ...
        print(
            f"[INFO] 初始化数据集: {self.dataset_name}, 路径: {root_folder}, 选择 {num_classes} 类"
        )

    @staticmethod
    def get_classification_transform(
        size: int, mode: DatasetSplit = DatasetSplit.TRAIN
    ):
        if mode == DatasetSplit.TRAIN:
            return transforms.Compose(
                [
                    transforms.RandomResizedCrop(
                        size, scale=(0.8, 1.0), interpolation=InterpolationMode.BICUBIC
                    ),
                    ...
                    transforms.RandomErasing(
                        p=0.25, scale=(0.02, 0.2), ratio=(0.3, 3.3), inplace=True
                    ),
                ]
            )
        else:
    ...
    def _subset_imagefolder(self, ds: ImageFolder) -> Subset:
        ...
        if not (1 <= self.num_classes <= len(ds.classes)):
            raise InvalidDatasetClassesNum(
                f"num_classes={self.num_classes} 越界，最大 {len(ds.classes)}"
            )
    ...
    def _filter_imagefolder_by_classes(
        self, ds: ImageFolder, kept_idx: Dict[str, int]
    ) -> SimpleImageDataset:
        ...
        return SimpleImageDataset(
            new_samples, classes, transform=ds.transform, loader=ds.loader
        )

    def _merge_sharded_imagefolders(
        self, base: str, shard_dirs: List[str], split: DatasetSplit, transform
    ):
        """
        ...
        for cls, i in kept_idx.items():
            classes[i] = cls
        return SimpleImageDataset(
            all_samples, classes, transform=transform, loader=loader
        )
    ...
    def _load_tinyimagenet(self, split: DatasetSplit):
        ...
        if split == DatasetSplit.TRAIN:
            ...
            train_classes = sorted(os.listdir(os.path.join(base, "train")))[
                : self.num_classes
            ]
    ...
    def _load_imagenet100(self, split: DatasetSplit):
        ...
        std_dir = os.path.join(base, split.value)
        ...
        raise FileNotFoundError(
            f"未找到 {split.value} 数据。需要 {std_dir} 或 {split.value}.X*"
        )
